    ...
    if request.method == 'POST':
        ...
        if user1 is not None: #驗證通過
            if user1.is_active: #帳號有效
                ...
                return redirect('/adminmain/') #開啟管理畫面
            else: #帳號無效
                message='帳號未啟用'
        else: #驗證失敗
            message='登入失敗'
...
def adminmain(request, pageindex=None): #管理頁面
	...
	if pageindex == None:
		page1=1
		newsunit=models.NewsUnit.objects.order_by('-id')[:pagesize]
	elif pageindex == '1':
		start=(page1-2)*pagesize
		if start >= 0:
			newsunit=models.NewsUnit.objects.order_by('-id')[start:(start+pagesize)]
			page1-=1
	elif pageindex == '2':
		start=page1*pagesize
		if start<datasize:
